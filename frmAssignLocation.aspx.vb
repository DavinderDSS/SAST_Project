Imports System.IO
Partial Class frmAssignLocation
            ...
            Try
                ...
                Select Case s(0).ToString
                    Case "cmbEmpCode"
                        ...
                        If DataDr.HasRows Then
                            ...
                            While DataDr.Read
                                ...
                                returnValue.Append(DataDr(0) & "~" & DataDr(1) & "~" & DataDr(2))
                            End While
                        End If
                        DataDr.Close()
                        DataDr = Nothing
                    ...
                    Case "DisplayMailID"
                        ...
                        Exit Sub
                    Case "SearchedData"
                        ''Call GetDataByDr("select emp_staffid,emp_staffid+' - '+User_initial as Emp  from Hamali_user_master where user_initial like '%" & s(1).ToString & "%' order by emp_staffid")
                        Call GetDataByDr("select distinct hum.emp_staffid,hum.emp_staffid+' - '+hum.User_initial as Emp from Hamali_user_master as hum inner join hamal_trans as ht on hum.emp_staffid=ht.supervisor_code where hum.user_initial like '%" & s(1).ToString & "%' and ht.approval='approve' and hum.empstatus='Active' order by hum.emp_staffid ")
                    ...
                    Case "SaveData"
                        ...
                        If hdnRndNo.Value = Session("RandomNumber") Then ''// code is added on 12-05-2020 For Request Flooding Issue Start here using existing code too
                            ...
                            'MsgBox(s(1).ToString())
                            If RAffected > 0 Then
                                returnValue.Append("SaveData|Location Updated Successfully")
                                Session("RandomNumber") = "" ''is used for Request Flooding Revalidation added this on 12-05-2020
                            Else
                                returnValue.Append("SaveData|Location Not Updated")
                                Session("RandomNumber") = "" ''is used for Request Flooding Revalidation added this on 12-05-2020
                            End If
